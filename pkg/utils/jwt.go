...
func CreateAuth(userid uint64, token *dao.Token) error {
	...
	errAccess := db.REDIS.Set(token.AccessUuid, strconv.Itoa(int(userid)), at.Sub(now)).Err()
	if errAccess != nil {
		return errAccess
	}
	errRefresh := db.REDIS.Set(token.RefreshUuid, strconv.Itoa(int(userid)), rt.Sub(now)).Err()
	if errRefresh != nil {
		return errRefresh
	}
	return nil
